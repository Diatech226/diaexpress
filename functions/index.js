...
exports.get_providers = onRequest(async (request, response) => {
  ...
  db.ref("/payment_settings").once("value", (psettings) => {
    if (psettings.val()) {
      const arr = [];
      const obj = psettings.val();
      const pms = Object.keys(obj);
...
exports.googleapi = onRequest(async (request, response) => {
  ...
  const json = await rgf.apiCallGoogle(request, settings, config);
  response.send(json);
});
...
exports.request_mobile_otp = onRequest(async (request, response) => {
  ...
  if (language) {
    ...
    const smsConfigData = await db.ref("smsConfig").once("value");
    const smsConfig = smsConfigData.val();
